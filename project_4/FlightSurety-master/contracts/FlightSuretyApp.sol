...
contract FlightSuretyApp {
    ...
    */
    modifier requireIsOperational() {
        ...
        require(isOperational(), "Ups! Contract is currently not operational");
    ...
    */
    modifier requireContractOwner() {
        require(msg.sender == contractOwner, "Ups! you are not the contract owner");
        _;
    }
    ...
    modifier requireNotAlreadyRegistered(address airline) {
        require(!flightSuretyData.isAirlineRegistered(airline), "Ups! Airline is already registered");
        _;
    }

    modifier requireFunded(address airline) {
        require(flightSuretyData.isFunded(airline), "Ups! Airline has not sufficiently contributed to the funds");
        _;
    }
     ...
     *
     */
    function registerAirline(address airline) public payable requireIsOperational requireNotAlreadyRegistered(airline) returns (bool success, uint256 votes) {
        if (flightSuretyData.getRegisteredAirlinesCount() < 4) {
            ...
            for (uint256 i = 0; i < airlinesForRegistration[airline].length; i++) {
            ...
            require(!isVoted, "Ups! Airline has already been voted for");
            airlinesForRegistration[airline].push(msg.sender);

            // Check if enough votes to register airline
            uint256 consensus = (airlinesForRegistration[airline].length * 100).div(flightSuretyData.getRegisteredAirlinesCount());
    ...
    function fundAirline() public payable {
        ...
        if (flightSuretyData.isFunded(msg.sender)) {
            emit AirlineFunded(msg.sender, flightSuretyData.getFunds(msg.sender));
        }
    }
    ...
    function buyInsurance(string memory flight, address airline) external payable requireIsOperational {
        // Check if amount range is greater than 0 ether and less than 1 ether.
        require((msg.value > 0 ether) && (msg.value <= 1 ether), "Ups! Insurance amount must be between 0 and 1 ether");
    ...
    *
    */
    function creditPassenger(string memory flight, address airline) external payable requireIsOperational {
        bytes32 flightKey = getFlightKey(flight, airline);
        uint256 amount = flightSuretyData.creditedAmount(msg.sender, flightKey);
        require(amount > 0, "Ups! No balance to withdraw");
    ...
    function creditedAmount(string memory flight, address airline) external view returns (uint256) {
        bytes32 flightKey = getFlightKey(flight, airline);
    ...
    */
    function registerFlight(string memory flight, address airline) external {
        bytes32 flightKey = getFlightKey(flight, airline);
        require(!flights[flightKey].isRegistered, "Ups! Flight is already registered");

        flights[flightKey] = Flight(true, STATUS_CODE_UNKNOWN, block.timestamp, airline);
    }

    function getFlightKey(string memory flight, address airline) internal pure returns (bytes32) {
    ...
    function processFlightStatus(address airline, string memory flight, uint256 timestamp, uint8 statusCode) public {
        require(address(0) != airline, "Ups! Airline address must be valid");
    ...
    function registerOracle() external payable {
        ...
        require(msg.value >= REGISTRATION_FEE, "Ups! Registration fee is required");
        uint8[3] memory indexes = generateIndexes(msg.sender);
        oracles[msg.sender] = Oracle({isRegistered : true, indexes: indexes});
    }

    function getMyIndexes() external view returns (uint8[3] memory) {
        require(oracles[msg.sender].isRegistered, "Ups! Not registered as an oracle");
    ...
    function submitOracleResponse(uint8 index, address airline, string memory flight, uint256 timestamp, uint8 statusCode ) external {
        require((oracles[msg.sender].indexes[0] == index) || (oracles[msg.sender].indexes[1] == index) || (oracles[msg.sender].indexes[2] == index), "Ups! Index does not match oracle request");

        bytes32 key = keccak256(
            abi.encodePacked(index, airline, flight, timestamp)
        );

        require(oracleResponses[key].isOpen, "Ups! Flight or timestamp do not match oracle request" );
