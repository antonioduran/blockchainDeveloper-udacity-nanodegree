...
contract FlightSuretyData {
    ...
    */
    ...
    modifier requireIsOperational() {
        require(operational, "Ups! Contract is currently not operational");
    ...
    modifier requireContractOwner() {
        require(msg.sender == contractOwner, "Ups! You re not the contract owner");
        _;
    }

    // airline modifiers
    modifier requireNotAlreadyRegistered(address airline) {
        require(!airlines[airline].isRegistered, "Ups! Airline is already registered");
        _;
    }

    modifier requireRegistered(address airline) {
        require(airlines[airline].isRegistered, "Ups! Airline is not registered");
        _;
    }

    modifier requireFunded(address airline) {
        require(airlines[airline].funds >= 10 ether, "Ups! Airline is not sufficiently contributed to the funds");
        _;
    }

    modifier requireAuthorizedCaller() {
        require(authorizedCallers[msg.sender], "Ups! Calling contract is not authorized to access data");
        _;
    }
    ...
    *
    */
    function fund(address airline, uint256 amount) public payable requireIsOperational requireRegistered(airline) {
     ...
     *
     */
    function buy(bytes32 flight, address airline, address passenger, uint256 amount ) external requireIsOperational {
        bool funded = airlines[airline].funds >= 10;
        require(funded, "Ups! Airline you are buying insurance from should have contributed to insurance funds");
        bytes32 insuranceKey = getInsuranceKey(passenger, flight);
        require(flightInsurances[insuranceKey].amount == 0, "Ups! Passenger is already insured");
    ...
    function creditedAmount(address passenger, bytes32 flight) public view returns (uint256) {
    ...
    function pay(address passenger, bytes32 flight) external returns (uint256) {
        ...
        require(amount > 0, "Ups! Credited amount should be greater than zero");
    ...
    function getInsuranceKey(address passenger, bytes32 flight) internal pure returns (bytes32) {
