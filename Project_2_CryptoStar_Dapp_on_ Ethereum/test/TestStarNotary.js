...
it('can Create a Star', async() => {
    ...
    await instance.createStar('Antonio Star', tokenId, {from: accounts[0]})
    assert.equal(await instance.tokenIdToStarInfo.call(tokenId), 'Antonio Star')
});
...
it('lets user1 put up their star for sale', async() => {
    ...
    await instance.createStar('Antonio Star', starId, {from: user1});
...
it('can add the star name and star symbol properly', async() => {
    ...
    let user1 = accounts[1];
    let tokenId = 6;
    await instance.createStar('Atreyu', tokenId, {from: accounts[0]});
  
    //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided

    assert.equal(await instance.tokenIdToStarInfo.call(tokenId), 'Atreyu');
    assert.equal(await instance.name(), "AntonioDuran");
    assert.equal(await instance.symbol(), "FADR");
});
...
it('lets a user transfer a star', async() => {
    ...
    let tokenId = accounts[1];
    let instance = await StarNotary.deployed();
    await instance.createStar('StarforTransferDiber', tokenId, {from: accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar("test", accounts[0], {from: accounts[0]});
    assert.equal(await instance.lookUptokenIdToStarInfo(1), "Antonio Star");
});
